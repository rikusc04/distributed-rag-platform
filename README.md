...
- **Semantic caching:** Redis-backed embedding-similarity cache with two layers — a chunk cache in front of pgvector retrieval, and an answer cache in front of the `ask` tool's LLM call. On a paraphrase-heavy benchmark this cuts `ask` p95 from 1266 ms to 285 ms and avoids ~99.65% of `gpt-4o-mini` completions. See [`bench/results/findings.md`](bench/results/findings.md) for methodology, numbers, and design notes.
...
- **Local, no AWS:** `./scripts/dev-up.sh` brings up postgres+pgvector, redis, and prometheus in docker for iterating on the gateway. Measured numbers from a k6 load test against that stack are in [`bench/results/findings.md`](bench/results/findings.md).
- **First time on AWS?** Start with the detailed walkthrough: [`docs/getting-started.md`](docs/getting-started.md). Assumes zero cloud knowledge.
